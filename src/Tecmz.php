...
class Tecmz
{
     ...
     * @return string
     */
    private function sign($params, $appSecret, $function = 'trim', $appSecretName = 'app_secret')
    {
    ...
    private function generate($code, $msg, $data = null)
    {
        $response = [
            'code' => $code,
            'msg' => $msg,
    ...
    private function request($gate, $param = [])
    {
        ...
        if ($this->appSecret) {
            ...
            $param['sign'] = $this->sign($param, $this->appSecret);
        }
        $url = self::$API_BASE . $gate;
        // print_r([$url, $param]);exit();
        if ($this->debug) {
            $this->log('TecmzApi -> ' . $url . ' -> ' . json_encode($param, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        }
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'TecmzApiSdk');
        $output = curl_exec($ch);
        curl_close($ch);
        $json = @json_decode($output, true);
        if (empty($json)) {
            return $this->generate(-1, '请求失败', [
                'output' => $output,
            ]);
        }
        return $json;
    ...
    public function ping()
    {
        ...
        if ($ret['code']) {
            return $this->generate(-1, 'PING失败');
        }
        return $this->generate(0, 'ok');
    }
